...
#import "HZMulticolorView.h"

@interface HZMulticolorView ()

@property (nonatomic, strong) CAShapeLayer *circleLayer;

@end

@implementation HZMulticolorView
...
#pragma mark - 配置圆形
- (CAShapeLayer *)produceCircleShapeLayer:(CGFloat)strokeStart strokeEnd:(CGFloat)strokeEnd{
    // 生产出一个圆的路径
    ...
    // 可以设置出圆的完整性
    _circleLayer.strokeStart = strokeStart;
    _circleLayer.strokeEnd = strokeEnd;
...
- (void)startAnimation:(CGFloat)strokeStart strokeEnd:(CGFloat )strokeEnd{
    ...
    // 生产一个圆形路径并设置成遮罩
    self.layer.mask = [self produceCircleShapeLayer:strokeStart strokeEnd:strokeEnd];
    ...
    if (_isReverse) {
        animation.fromValue         = [NSNumber numberWithDouble:M_PI*2];
